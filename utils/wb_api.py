...
async def wb_check_token(wb_token) -> bool:
...
async def wb_check_current_price(wb_token, wb_nm_id) -> int:
    response = requests.get(url=f'{wb_url}/api/v2/list/goods/filter', headers={'Authorization': wb_token},
...
async def wb_change_price(wb_token, wb_nm_id, target_price) -> bool:
